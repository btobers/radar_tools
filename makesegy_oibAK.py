"""
converts hdf5 radar data to seg-y format
adapted from github.com/mchristoffersen/segygen
...
import pandas as pd
import h5py
import struct, sys, multiprocessing
import os.path
...
def main():
  ...
  df = pd.read_csv(sys.argv[1], sep="\t", header=None, names=['ids'])
  ids = df['ids']
  # Make list of paths for segy generation
  datapfix = "/home/btobers/Documents/data/radar/mala/"
  outpfix = "/mnt/d/MARS/targ/supl/UAF/segydB/"

  paths = []
  for id in ids:
    insubdir = id[0:4] + "/hdf5/"
    insubdir = ""
    datafile = id + ".h5"
    outfile = id + ".sgy"
    paths.append((datapfix+insubdir+datafile,outpfix+outfile))

  sgyexist = []
  datadne = []
  fpaths = []
  for i in range(len(paths)):
    if(not os.path.isfile(paths[i][0])):
      datadne.append(ids[i])
      continue
    fpaths.append(paths[i])

  paths = fpaths
  pool = multiprocessing.Pool(nproc)
  pool.map(gensegy, paths)

  if(len(datadne) > 0):
    print("")
    print("Some data files could not be found:")
    for i in datadne:
      print(i)

  #if(len(sgyexist) > 0):
  #  print("")
  #  print("Some segy files already exist:")
  #  for i in sgyexist:
  #    print(i)
...
def build_binhead(spt, dt, ntrace):
  ### Build binary header
  binhead = b''
  ...
  # (16) Sample interval
  binhead += struct.pack(">H",int(dt*1e9))
...
def gensegy(paths):
  ...
  fn = data_path.split("/")[-1][:-3]
  # pull necessary raw group data
  spt = int(f["raw"]["rx0"].attrs["samplesPerTrace"])                   # samples per trace in rgram
  ntrace = int(f["raw"]["rx0"].attrs["numTrace"])                       # number of traces in rgram 
  dt = 1/ f["raw"]["rx0/"].attrs["samplingFrequency-Hz"]                # sampling interval, sec
  data = f["drv/proc0"][:]                                              # pulse compressed array
  ...
  with open(out_path,'wb') as f:
    ...
    f.write(build_txthead(fn))

    ### Write 400 byte binary header
    f.write(build_binhead(spt, dt, ntrace))

    ### Write data traces
    for i in range(ntrace):
      #print(int(lat[i]), int(lon[i]))
      f.write(build_trchead(spt,i+1,int(lat[i]),int(lon[i])))
      datadB = data[i,:]
      datadB[datadB <= 0] = .000001
      datadB = 10*np.log10(data[i,:])
      f.write(datadB.astype(">f", order='C').tobytes(order='C'))

  print("Generated segy {}".format(out_path.split('/')[-1]))
